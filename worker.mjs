...
function drawFighter(f, mine){
  ...
  ctx.lineWidth = 3;
  inkCircle(X(P.head), Y(P.head), P.hr, sd + 10);
  ctx.fillStyle = f.ink;
  const ex = X(P.head) + fc * 4, ey = Y(P.head) - 2;
  if (f.state === 'ko'){
    ctx.lineWidth = 2;
    inkLine(ex - 3, ey - 3, ex + 3, ey + 3, sd + 11);
    inkLine(ex + 3, ey - 3, ex - 3, ey + 3, sd + 12);
  } else { ctx.beginPath(); ctx.arc(ex, ey, f.state === 'hit' ? 2.6 : 1.8, 0, 7); ctx.fill(); }
  ctx.restore();
}
...
export class Arena extends DurableObject {
  ...
  drop(ws){
    ...
    if (this.seats[0] === ws) this.seats[0] = null;
    else if (this.seats[1] === ws) this.seats[1] = null;
